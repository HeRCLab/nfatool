...
void dump_dot_file2 (char *filename, xmlNode *aNode, vector<int> subset, int colors) {
  FILE *myFile;
  int found,found2;
  char str[1024];
...
*/
  for (int i=0;i<virtuual_root_edges.size();i++) {
    for (int j=0;(j<max_edges) && (graph[i][j]!=-1);j++) {
        found=0;
        found2=0;
        for (int k=0;k<subset.size();k++) {
          if (subset[k]==i) found=1;
          if (subset[k]==graph[i][j]) found2=1;
        }
