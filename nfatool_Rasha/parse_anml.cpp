...
//			  printf("%d\n", edge_table[i][j]);
			  if(edge_table[i][j]==i) break;  
			  u++; 
			  m = edge_table[i][j]; 			  
//			  rev_edge_count[m]=u;
			  if(u > max_fan_in) max_fan_in = u; //if (rev_edge_count[edge_table[i][j]] > max_fan_in) max_fan_in = rev_edge_count[edge_table[i][j]];
			}
		}
	}
	...
	for (int i = 0; i < num_states; i++){
	  for (int j = 0; j < max_edges; j++){
		if(edge_table[i][j] == -1){
		  ...
		  my_reverse_table[edge_table[i][j]][u]=i ; //rev_edge_count[edge_table[i][j]]++] = i;
		}
	  }
	}
